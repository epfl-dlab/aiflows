...
def _create_retry_decorator(llm: SafeChatOpenAI) -> Callable[[Any], Any]:
    import openai
...
async def acompletion_with_retry(llm: SafeChatOpenAI, **kwargs: Any) -> Any:
...
class SafeChatOpenAI(BaseChatModel):
    """Edited version of langchain.chat_models.ChatOpenAI to enable multithread calls
        What's new:
            - We are no longer editing the openai variables (e.g. openai.api_type)
            - api information are initialized upon input and is stored in the object (e.g. self.api_type)
            - When calling the client (default: openai.ChatCompletion), we explicitly pass in the api
                 information via object attributes to avoid race conditions with the global openai variable
    """
