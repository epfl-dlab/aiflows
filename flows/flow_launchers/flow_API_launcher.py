...
class FlowLauncher(MultiThreadedAPILauncher):
    ...
    def predict(self,
        ...
        batch = self.predict_batch(flow = flow,
    ...
    def predict_batch(
        ...
        for sample in batch:
            for _sample_idx in range(n_independent_samples):
                ...
                output_message, output_data, _error = cls.predict_sample(flow = flow,
                                                                    sample=sample,
                                                                    input_interface= input_interface,
                                                                    output_interface= output_interface,
    ...
    def launch(cls,
        ...
        data = cls.predict_batch(flow = flow,
                            input_interface = input_interface,
                            output_interface = output_interface,
                            batch = data,
                            path_to_output_file = path_to_output_file,
