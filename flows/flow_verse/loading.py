...
def write_or_append_gitignore(local_dir: str):
    gitignore_path = os.path.join(local_dir, ".gitignore")
    with open(gitignore_path, "a") as gitignore_f:
        lines = [
            "\n\n\n# auto-generated by flows\n",
            "./*\n"
        ]
...
def fetch_local(file_path: str, revision: str, flow_mod_id: str, cache_dir: str, local_dir: str):
    shutil.copytree(file_path, local_dir, ignore=shutil.ignore_patterns(".git"))
    # write the revision info in the folder
    write_mod_id(local_dir, flow_mod_id)
...
def build_mod_id(repo_id_or_file_path: str, revision: str):
    ...
    if match is not None:
        raise ValueError(f"revision {revision} is not a valid revision, it contains illegal character: {match.group(0)}")
...
def sync_dependency(dep: Dict[str, str], overwrite: bool=False):
    ...
    if os.path.exists(url): # url is a local path
        ...
        print(f"dependency url {file_path} is a local directory, the revision {revision} will be ignored")

        basename = os.path.join("local", file_path.split("/")[-1])
        ...
        if not os.path.isdir(local_dir) or overwrite:
            fetch_local(file_path, revision, mod_id, DEFAULT_CACHE_PATH, local_dir)
        else:
            # TODO(yeeef): use logger rather than print afterwards
            print(f"dependency url {url} with revision {revision} already synced, skip")
...
def sync_dependencies(dependencies: List[Dict[str, str]], overwrite: bool=False):
    ...
    write_or_append_gitignore(flow_module_dir)

    for dep in dependencies:
        dep = validate_and_augment_dependency(dep)
        sync_dependency(dep, overwrite)
