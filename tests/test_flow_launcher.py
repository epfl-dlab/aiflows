...
def test_get_outputs_to_write():
    batch = [{"id": 0, "inference_outputs": "test 0"}, {"id": 1, "inference_outputs": "test 1", "success": False}]
...
def test_init_multithreaded_launcher():
    ...
    config = {"api_keys": ["key1", "key2"], "single_threaded": True}
    launcher = MultiThreadedAPILauncher(**config)
...
def test_predict_dataloader(monkeypatch):
    ...
    config = {"api_keys": ["key1", "key2"], "debug": True}
    ...
    results = launcher.predict_dataloader(
        data, [{"id": 0, "inference_outputs": "test 0", "success": True, "error": "None"}]
    )
    ...
    config = {"api_keys": ["key1", "key2"], "debug": False}
    launcher = MockMultithreadedLauncher(**config)
