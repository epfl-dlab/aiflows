...
import flows
from flows.flow_verse.loading import _sync_dependencies, FlowModuleSpecSummary, FlowModuleSpec, DEFAULT_FLOW_MODULE_FOLDER, NO_COMMIT_HASH, is_sync_dir_modified, add_to_sys_path
...
def mock_sync_flow_dependencies(tmpdir, cache_root):
    with mock.patch("flows.flow_verse.sync_dependencies", 
                    new=functools.partial(_sync_dependencies, flow_modules_base_dir=tmpdir, cache_root=cache_root, caller_module_name="<test>", all_overwrite=False)) as mock_sync:
        yield mock_sync
...
def local_sync_1f_UsefulChatBots(tmpdir, cache_root, sync_root, local_dev_path):
    ...
    correct_summary = FlowModuleSpecSummary(
        ...
        cache_root,
        mods=[
            FlowModuleSpec("1f/UsefulChatBots", 
                           local_dev_path, 
                           NO_COMMIT_HASH, 
                           local_dev_path,
...
def test_single_remote_overwrite(tmpdir, cache_root, sync_root, remote_sync_yeeef_UsefulChatBots: Tuple[FlowModuleSpecSummary, FlowModuleSpecSummary]):
    flow_mod_summary, correct_summary = remote_sync_yeeef_UsefulChatBots
    assert_flow_module_summary(flow_mod_summary, correct_summary)
...
def test_remote_revision_to_local(tmpdir, cache_root, sync_root, remote_sync_yeeef_UsefulChatBots: Tuple[FlowModuleSpecSummary, FlowModuleSpecSummary], local_dev_path: str):
...
def test_local_revision_to_remote(tmpdir, cache_root, sync_root, local_sync_yeeef_UsefulChatBots: Tuple[FlowModuleSpecSummary, FlowModuleSpecSummary]):
...
def test_sync_dependency_chain(tmpdir, cache_root, sync_root, remote_sync_yeeef_UsefulChatBots: Tuple[FlowModuleSpecSummary, FlowModuleSpecSummary]):
    ...
    new_flow_mod_summary = FlowModuleSpecSummary.from_flow_mod_file(
        os.path.join(sync_root, "flow.mod")
    )
    correct_summary = FlowModuleSpecSummary(
        sync_root,
        cache_root,
