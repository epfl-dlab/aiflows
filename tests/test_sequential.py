...
def atomic_flow_builder(bias):
    ...
    return MyFlow(name="my-flow", description="flow-sum", output_keys=["v0"], input_keys=["v0"], bias=bias)
...
def test_basic_instantiating() -> None:
    ...
    flow = SequentialFlow(
        name="name", description="description", input_keys=["v0"], verbose=False, dry_run=True, flows=[flow_a, flow_b]
    )
...
def test_basic_call():
    ...
    task_message = seq_flow.package_task_message(
        recipient_flow=seq_flow, task_name="task", task_data=data, output_keys=["v0"]
    )
...
def test_early_exit(monkeypatch, caplog):
    ...
    data = {"v0": 10}
    task_message = seq_flow.package_task_message(
        recipient_flow=seq_flow, task_name="task", task_data=data, output_keys=[]
    )
...
def test_pass_on_api_key():
    ...
    data = {"v0": 10}
    task_message = seq_flow.package_task_message(
        recipient_flow=seq_flow, task_name="task", task_data=data, output_keys=["v0", "mock_flow_api_key"]
    )
