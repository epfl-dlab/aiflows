...
if __name__ == "__main__":
    ...
    assert (
        human_readable_outputs[0]["observation"].startswith("Could not find [President]")
        and human_readable_outputs[0]["wiki_content"] is None
    )
    assert (
        human_readable_outputs[1]["observation"].startswith("The president of India (IAST: Bhārat kē Rāṣṭrapati)")
        and human_readable_outputs[1]["wiki_content"] is not None
    )
